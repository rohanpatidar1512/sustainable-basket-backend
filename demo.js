...
app.get("/api/pay", async (req, res) => {
  ...
  const payload = {
    ...
    redirectUrl: `https://localhost:5000/redirect-url/${merchantTransactionId}`,
    redirectMode: "REDIRECT",
    callbackUrl:  "https://www.sustainablebasket.com/product-category/beauty-and-health/",
    paymentInstrument: {
      type: "PAY_PAGE"
    },
...
app.get("/api/redirect-url/:merchantTransactionId", (req,res) => {
  ...
  if(merchantTransactionId) {
    ...
    axios.request(options)
      .then(async function (response) {
        console.log(response.data);
        // If payment is successful, store payment info in the order
        if (response.data.code === "PAYMENT_SUCCESS") {
          // Assuming you extract necessary payment information from response.data
          const paymentInfo = {
          ...
          try {
            ...
            const newOrder = new Order({
              // Populate with other required fields
              paymentInfo: [paymentInfo]
            });
            // Save the order to the database
            const savedOrder = await newOrder.save();
            console.log("Order saved with payment info:", savedOrder);
          } catch (error) {
